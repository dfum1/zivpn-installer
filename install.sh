...
echo "==============================="
echo " ZiVPN UDP + Dashboard Installer"
echo "==============================="
...
apt install -y curl wget python3 python3-pip jq cron

pip3 install --no-cache-dir rich

# ---- STEP 3: INSTALL ZIVPN UDP SERVER ----
echo "[3/6] Downloading ZiVPN UDP server..."
cd /root || exit 1
wget -O zi.sh https://raw.githubusercontent.com/zahidbd2/udp-zivpn/main/zi.sh
chmod +x zi.sh

echo "-----------------------------------"
echo " ZiVPN installer will start now."
echo " When asked, PRESS ENTER."
echo "-----------------------------------"
./zi.sh
...
def sync():
    pw=[p for p,_ in load_db()]
    tmp="/tmp/zivpn.json"
    subprocess.run(
        ["jq",f'.password={json.dumps(pw)}',CONFIG],
        stdout=open(tmp,"w"),
        check=True
    )
...
# ---- STEP 5: CRON AUTO CLEAN ----
echo "[5/6] Setting up auto-clean cron..."
(crontab -l 2>/dev/null; echo "*/5 * * * * /usr/bin/python3 /usr/local/bin/zivpn <<< 4 >/dev/null 2>&1") | crontab -

# ---- DONE ----
echo "==============================="
echo " INSTALLATION COMPLETED"
echo "==============================="
echo "Run command: zivpn"
echo "==============================="
